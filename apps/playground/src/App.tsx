...
export function App() {
  ...
  const drag = useGlassDrag(state.example);
  ...
  const matchOpticalLayout = () => {
    const r = drag.stage.current?.getBoundingClientRect();
    if (!r) return;
  ...
  useEffect(() => {
    if (!state.referenceLayout || !drag.stage.current) return;
    const observer = new ResizeObserver(matchOpticalLayout);
    observer.observe(drag.stage.current);
    matchOpticalLayout();
    return () => observer.disconnect();
  }, [state.referenceLayout, drag.stage]);
        ...
        <div className="main-column">
          <section className="preview-panel" aria-label="Live preview">
            ...
            <div className="drag-toolbar">
              ...
              <button
                onClick={drag.reset}
                disabled={drag.position.x === 0 && drag.position.y === 0}
              >
                Center
              </button>
            </div>
