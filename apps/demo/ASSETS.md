...
The owner selected the Join concept: two diagonal lobes with a continuous neck. The production mark is an original single-path SVG with tangent circular arcs, a slightly larger lower lobe and no gradient or stroke. `src/lib/brand.ts` is the geometry/color source; `BrandMark` renders it decoratively next to accessible brand text.

The outlined lockup text derives from the bundled Inter 600 font under its retained OFL license. Logo exports, SVG/ICO favicons, Apple touch icon, regular/maskable app icons and social images are generated by `pnpm brand:generate`. See [brand usage](docs/brand.md).

`public/brand/material-scene.png` is a cropped screenshot of the actual running demo at 1440x900, captured September 9, 2026 with the real LiquidGlass component visible. The README/OpenGraph/Twitter banner combines it with the approved mark and outlined wordmark. The screenshot is not an AI-painted glass layer.
