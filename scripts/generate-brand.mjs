...
import { BRAND_COLOR, BRAND_MARK_PATH } from "../apps/demo/src/lib/brand.ts";
...
const tile = (maskable = false, rounded = false) => svg(
  `<rect width="32" height="32"${rounded ? ' rx="7"' : ""} fill="${BRAND_COLOR}"/>` +
  (maskable ? `<g transform="translate(4 4) scale(.75)">${mark("#fff")}</g>` : mark("#fff")),
);
...
await writeFile(join(app, "icon.svg"), tile(false, true) + "\n");
for (const size of [192, 512]) await writeFile(join(brand, `icon-${size}.png`), await png(tile(), size));
await writeFile(join(brand, "icon-maskable-512.png"), await png(tile(true), 512));
await writeFile(join(app, "apple-icon.png"), await png(tile(), 180));

// PNG entries in an ICO container preserve the exact small-size renders.
const sizes = [16, 32, 48, 64, 96];
const entries = await Promise.all(sizes.map((size) => png(tile(false, true), size)));
...
try {
  ...
  const background = svg(`<rect width="1200" height="630" fill="#fafafb"/>
    <g transform="translate(56 148) scale(2.5)">${mark(BRAND_COLOR)}</g>
  ...
  const banner = await sharp(Buffer.from(background)).composite([{ input: photo, left: 606, top: 40 }]).png().toBuffer();
  ...
  const alt = "React Glaze - Liquid glass for React. The Join logo beside a real glass component over a cobalt ceramic and coral textile scene.\n";
